...
main(int argc,char ** argv)
{
	...
	struct sockaddr_in kliens2;
	int kliens2_meret = sizeof(kliens2);
	char on = 1;
	char szerver_cim[16];
	int flags = 0;
	...
	int connInt = connect(fd,(struct sockaddr*) &szerver,szerver_meret);
   	// error(fd,"Failed to connect to server.");
   	...
   	while (1)
   	{
			...
			send(fd,&r,sizeof(r),flags);

	   		printf("uzenet elkuldve\n");
	   		int send_rnr = send(fd,&rnr,sizeof(rnr),flags);
	   		error(send_rnr,"Failed to send the message. sign");
	   	// PENZ KULDESE

	   		int recv_petak = recv(fd,&petak,sizeof(petak),flags);
			...
			printf("/ - BALANCE - / : %d\n",petak -tet);
		// TET KULDESE	
		   	int send_tet = send(fd,&tet,sizeof(tet),flags);
	   		error(send_tet,"Failed to send the message.");
   		// SZELVENY KULDESE
	   		int send_szelveny = send(fd,szelveny,sizeof(szelveny),flags);
	   	...
	   	// NYEROSZAM FOGADASA
			...
			// memset(fogadoTomb,0,MAX_NYSZ_COUNT);
		   	...
		   	int rec_nyeroszam = recv(fd,nyeroszamok,MAX_NYSZ_COUNT*sizeof(int),flags);
			...
			// 	}
	   		...
	   		// }
		// PENZ FOGADASA
			int rec_petak2 = recv(fd,&petak,sizeof(petak),flags);
		   	error(rec_petak2,"Failed to receive the message.22");
		// UZENET FOGADASA	
		   	if (petak < 1)
		   	{
		   		printf("You ran out of money!\n");
		   		int rec_uzi = recv(fd,uzi,sizeof(uzi),flags);
